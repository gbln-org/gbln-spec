...
**Design Goals:**
1. **LLM-optimised**: 84% fewer tokens than JSON for AI contexts via I/O format
2. **Deterministic**: O(1) lookahead parsing, no ambiguity
3. **Type-safe**: Parse-time validation prevents runtime errors
4. **Memory-efficient**: Bounded types reduce RAM usage, I/O format with XZ compression
...
#### 4. Whitespace Rules and I/O Format

**Whitespace is significant ONLY inside value delimiters `()`.**

**GBLN distinguishes between two file formats:**

1. **Human-Editable Format (`.gbln`)**: 
   - Whitespace for readability
   - Comments preserved
   - Version controlled (Git)
   - Example: `config.gbln`

2. **I/O Format (`.io.gbln.xz`)**: 
   - MINI GBLN (no structural whitespace)
   - Comments stripped
   - XZ compressed (optional)
   - NOT version controlled
   - Generated via `gbln write` command

**Whitespace Removal Algorithm (MINI GBLN):**
```
For each character:
    IF inside '(' ... ')':
...
```gbln
:| Human-Editable Format (.gbln)
user{
    id<u32>(12345)
    name<s64>(Alice Johnson)
}

:| I/O Format - MINI GBLN (.io.gbln before XZ compression)
user{id<u32>(12345)name<s64>(Alice Johnson)}
```

**Key Points:**
- `(Alice Johnson)` preserves the space (part of value)
- Whitespace between `}` and `name` is structural (removed in MINI GBLN)
- Both formats parse to identical `Value` representation
- Transformation is lossless and reversible (source ↔ I/O via `gbln write`/`gbln read`)

**I/O Format Generation:**
```
Source File (.gbln)
    ↓ gbln write
MINI GBLN (.io.gbln) - 17% smaller
    ↓ XZ compress
Compressed I/O (.io.gbln.xz) - 73% smaller than original
```

**See Also:**
- Section "File Extensions" (Appendix B) for file naming conventions
- `docs/04-llm-optimisation.md` for detailed I/O format benefits
...
## Appendix B: MIME Type and File Extensions
...
GBLN uses a dual-file system to separate human-editable sources from optimised I/O files:

#### `.gbln` - Human-Editable Source
- **Purpose**: Edited by developers, committed to version control
- **Format**: Pretty-printed with 2-space indentation (configurable)
- **Comments**: Preserved
- **Whitespace**: Present for readability
- **Version Control**: YES (commit to Git)
- **Example**: `config.gbln`, `data.gbln`

#### `.io.gbln` - MINI GBLN (Intermediate)
- **Purpose**: Optimised for processing without compression
- **Format**: MINI GBLN (no structural whitespace, comments stripped)
- **Compression**: None
- **Version Control**: NO (add to .gitignore)
- **Generated By**: `gbln write --no-compress`
- **Example**: `config.io.gbln`
- **Typical Use**: Debugging, when XZ compression is unwanted
...
- **Version Control**: NO (add to .gitignore)
- **Generated By**: `gbln write` (default behaviour)
- **Example**: `config.io.gbln.xz`
- **Typical Use**: Production deployments, API transmission, LLM contexts
...
**Pattern**: `<basename>.gbln` → `<basename>.io.gbln.xz`

**Examples:**
- `config.gbln` → `config.io.gbln.xz`
- `users.gbln` → `users.io.gbln.xz`
- `api-response.gbln` → `api-response.io.gbln.xz`

### Workflow

```
1. Developer edits: config.gbln (human-readable)
2. Generate I/O file: gbln write config.gbln
   → Creates: config.io.gbln.xz (MINI + XZ compressed)
3. Application reads: config.io.gbln.xz (optimised)
4. Update source: gbln read config.gbln
   → Reads config.io.gbln.xz, updates config.gbln
```
